...
export default function Edit({ stampType, categories, colors }: Props) {
    const { data, setData, patch, processing, errors } = useForm<FormData>({
        name: stampType.name,
        icon: stampType.icon || '',
        color: stampType.color || '#6B7280',
        category: stampType.category,
    });

    const [iconPreview, setIconPreview] = useState(stampType.icon || '🏷️');

    useEffect(() => {
        setIconPreview(data.icon || '🏷️');
    }, [data.icon]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(route('master.stamp-types.update', stampType.id));
    };
    ...
    return (
        <AppLayout title={`スタンプ種類を編集: ${stampType.name}`}>
            <Head title={`スタンプ種類を編集: ${stampType.name}`} />
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900">スタンプ種類を編集</h1>
                    <p className="mt-1 text-sm text-gray-600">
                        「{stampType.name}」の設定を変更します
                    </p>
                </div>
                ...
                <div className="bg-white shadow rounded-lg p-6 mb-8">
                    ...
                    <div className="flex space-x-6">
                        ...
                        <div className="flex-1">
                            <p className="text-sm text-gray-500 mb-2">変更前:</p>
                        ...
                        {/* 変更後 */}
                        <div className="flex-1">
                            <p className="text-sm text-gray-500 mb-2">変更後:</p>
